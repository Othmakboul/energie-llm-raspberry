...
with ctx_prise as prise:
    for config in MODELES:
        ...
        print(f"\n===== {config['nom']} {config['quantification']} n_threads={config['n_threads']} =====")
        ...
        for max_tokens in MAX_TOKENS_VALEURS:
            for fiche in fiches:
                ...
                for run in range(N_REPETITIONS):
                    ...
                    print(f"[{config['quantification']}] [threads={config['n_threads']}] [max_tokens={max_tokens}] "
...
chemin = f"data/raw/nthreads_{MACHINE}_{horodatage}.csv"
